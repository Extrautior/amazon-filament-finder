function exportRows(payload) {
  ...
  return sections.flatMap((section) => {
    const cheapestRows = (payload.resultsByMaterial[section.key] || []).map((item, index) => ({
      ...
      spoolKg: item.spoolKg ?? "",
      totalKg: item.totalKg ?? "",
      pricePerKg: item.pricePerKg ?? "",
      currency: item.currency || "",
      freeShipping: item.freeShipping ? "Yes" : "No",
      shippingStatus: item.shippingStatus || "",
      freeShippingKind: item.freeShippingKind || "",
      minimumFreeShippingQuantity: item.minimumFreeShippingQuantity ?? "",
      freeShippingSubtotal: item.freeShippingSubtotal ?? "",
      shippingAtQuantityOne: item.shippingAtQuantityOne ?? "",
